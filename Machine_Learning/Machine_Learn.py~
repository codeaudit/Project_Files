        ...
        if dclass:
            ...
            for j in self.__document_classes:
                ...
                for i in d.Words():
                    ...
                    r = wf * sum_dclass / float(wf_dclass * sum_j)
                    prod *= r
...
DClasses = ["Psychology",  "Personal",  "Math",  "Machines"]
...
open('results.tsv', 'w').close()
f = open('results.tsv', 'w')
#f.write("Filename\tProbabilities\t Actual Class\n")
    ...
    for subdir, dirs, files in os.walk(base):
        for dir in dirs:
            for file in os.listdir(base+dir):
                res = p.Probability(base+dir + "/" + file)
                print(res)
                f.write(file+"\t"+str(res)+"\t"+dir+"\n")
f.close()


columns = ["Filename","Probabilities", "Actual Class"]
df = pd.read_csv('results.tsv', names=columns, sep='\t')
